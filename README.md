# maXMLian
maXMLian is a high-level stream-based XML parser for Java with a DOM-like API. Its performance is in the same order as the one of a StAX parser, but due to its high-level API it is much more suited for parsing XML trees with a complex structure, particularly when only certain parts of the document tree are of interest.
...
maXMLian targets on large XML files with a complex structure, i.e., when performance cannot be neglected, but using low-level parsers would result in cumbersome code. The following table shows recommendations which parser to use in which situation:
...
*Warning:* maXMLian is currently based on the StAX API. During tests with different StAX parser implementations we have observed that:

* All StAX parser implementations we have considered behave differently in some situations (see [Known Inconsistencies](#known-inconsistencies))
...
* We used the class `LargeXmlFileGenerator` in the module `maXMLian-xml-generator` to generate such an XML file.
* We used the class `ParserBenchmarkSuite` from the module `maXMLian-benchmarks` to benchmark the following parsers:
    ...
    * maXMLian XML parser (different modes)
* With each parser, we traversed the whole XML file including attributes.
* For the benchmark, maXMLian used Woodstox as internal StAX parser (see [Dependency on StAX parser](#dependency-on-stax-parser) for details).
...
See Section [Benchmark](#benchmark) for further details about how the benchmark works.
...
The fastest maXMLian variant is approximately 60 % slower than Aalto XML with the cursor API and as fast as the Xerces StAX parser with the iterator API. Reusing instances makes sense, but it only saves approximately 5 % of the runtime.     
...
For the first diagram, the garbage collector has not been triggered before measuring the memory consumption, which does not mean that the garbage collector has not been triggered otherwise. This gives an impression of how much memory is allocated temporarily and released later. The higher the peaks, the more time has been spent on memory allocations and garbage collection. Reusing instances obviously lowers the peaks.
...
maXMLian is currently based on a StAX parser. Since StAX parsers should be interchangeable, maXMLian has no hard dependency to a certain StAX parser implementation. However, it tries to use the following parsers in the given order:

1. Woodstox
1. Xerces
1. default StAX parser returned from ```XMLInputFactory.newFactory()``` (currently same as Xerces)

The reason for this prioritization although StAX parsers should be interchangeable is explained in [Differences between StAX Parsers](#differences-between-stax-parsers).

Using a StAX parser internally is an implementation detail. We might decide to write our own StAX parser (unlikely) or to change to another XML parsing framework under the hood. This is why there is no way to configure which StAX parser is used via API. The only way to influence this decision is to use Woodstox in your project or not.
...
* Character events: StAX distinguishes between character, space, and CDATA events. The return value of ```Text.isElementContentWhitespace()``` depends on the type of character event. We have observed cases where Aalto detects a different type of character event than Xerces and Woodstox do.  
...
Theoretically it would be possible for maXMLian to support random access if the XML could be accessed randomly. Random access here means to be able to process a node after the parser has parsed beyond it. Such access would mean that the internal parser state has to be restored. Since the goal of stream-based parsers is to avoid holding the whole XML or a comparable amount of data in memory, it is not possible to take a snapshot of the parser state for each node, but only with an acceptable frequency. In order to move to a certain node, one would move to the latest snapshot before this node and start parsing from there instead of from the beginning of the XML. Unfortunately, we are not aware of any StAX parser that supports storing its state and restoring it later.  

# Open Source License Acknowledgement

maXMLian itself does not depend on any third-party libraries, but the benchmark and unit test modules depend on the following libraries:

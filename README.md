...
* Character events: StAX distinguishes between character, space, and CDATA events. The return value of `Text.isElementContentWhitespace()` and `Node.getTextContent()` depends on the type of the character event. We have observed cases where Aalto signals a different type of character event than Xerces and Woodstox do. We have also observed cases in which Xerces reports character events instead of space events when the file has Unix or Mac line breaks and coalescing is activated. 
...
We have implemented some workarounds to deal with these parser inconsistencies and our unit tests ensure that maXMLian behaves like the Xerces DOM parser for all our test files for both, the Xerces and the Woodstox StAX parser, and Windows line breaks. Nevertheless, you should particularly be careful when evaluating the document type, the result of `Text.isElementContentWhitespace()`, or `Node.getTextContent()` with respect to leading and trailing whitespaces.
